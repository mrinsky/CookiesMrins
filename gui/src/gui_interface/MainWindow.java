...
//endregion
            ...
            public void windowClosing(WindowEvent windowEvent) {
                if (!isGuestMode) {
                    try {
                        ...
                        new XmlFileWorking().saveAllRU(Resources.traditions, Resources.countries, Resources.holidays);
    ...
    private class removeAllListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent event) {
            try{int indexInListTradition = 0;
            for (int i = 0; i < traditionTable.getRowCount(); ) {
                if (traditionTable.getValueAt(i, 4) == Boolean.TRUE) {

                    Resources.clientOut.println(xmlFileWorking.sendIdToServer_Remove(indexInListTradition));
                    ;
